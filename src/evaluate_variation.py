...
# Center-distance is used only when candidate scores are essentially tied.
...
def localize(reference_path, search_path):
    ...
    # Restrict the required center-distance tie-break to true score ties.
    ...
    for candidate in near_best_candidates:
        ...
        candidate["center_x"] = (
            candidate["top_left_x"] + size / 2.0
        )

        candidate["center_y"] = (
            candidate["top_left_y"] + size / 2.0
        )
...
for pair_index in range(30):
    ...
    print(
        ...
        f"candidates={prediction['near_best_candidates']}"
    )

results_path = os.path.join(
    RESULTS_FOLDER,
    "variation_evaluation_tight_tie_break.csv"
)
...
errors = np.array(
    [item["error_pixels"] for item in all_results],
    dtype=np.float32
)

runtimes = np.array(
    [item["runtime_ms"] for item in all_results],
    dtype=np.float32
)
...
print()
print("========== TIGHT TIE-BREAK SUMMARY ==========")
print("Tie-score ratio:", TIE_SCORE_RATIO)
