...
def create_layout(image_size=11000, seed=4040):
    rng = np.random.default_rng(seed)

    image = np.full((image_size, image_size), 35, dtype=np.uint8)

    for y in range(40, image_size, 80):
        cv2.line(image, (0, y), (image_size - 1, y), 155, 5)

    for x in range(35, image_size, 70):
        cv2.line(image, (x, 0), (x, image_size - 1), 120, 5)
    ...
    # Low-frequency local context surviving downsampling
    for _ in range(70):
        cx = int(rng.integers(500, image_size - 500))
        cy = int(rng.integers(500, image_size - 500))
        patch_size = int(rng.integers(250, 650))
        half = patch_size // 2

        x1 = max(0, cx - half)
        x2 = min(image_size, cx + half)
        y1 = max(0, cy - half)
        y2 = min(image_size, cy + half)
        ...
        cv2.ellipse(
            ...
            (width // 2, height // 2),
            (
                int(width * rng.uniform(0.25, 0.45)),
                int(height * rng.uniform(0.25, 0.45))
            ),
        ...
        shift = float(rng.uniform(-50, 50))

        patch = image[y1:y2, x1:x2].astype(np.float32)
        patch = patch + shift * (mask.astype(np.float32) / 255.0)
...
def add_noise(image, sigma, poisson_strength, rng):
    image_float = image.astype(np.float32)
...
def create_pair(index, layout, rng):
    ...
    # Bigger reference: 3000×3000 at high resolution.
    # It becomes a 300×300 template inside the 1000×1000 search image.
    reference_size_hr = 3000
...
print("Generating 30 large-context reference pairs...")

rng = np.random.default_rng(5050)
metadata = []

for index in range(30):
    item = create_pair(index, layout, rng)
...
print()
print("Large-context dataset complete.")
